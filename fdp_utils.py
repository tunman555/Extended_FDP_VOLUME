...
def gen_plane(p1,p2,h):
    lower = h[0]
    upper = h[1]
    x1,y1 = p1[0],p1[1]
    x2,y2 = p2[0],p2[1]
    
    X = np.array([[x1, (x1 + x2)/2 , x2],
       [x1, (x1 + x2)/2 , x2],
       [x1, (x1 + x2)/2 , x2]])
    
    Y = np.array([[y1, (y1 + y2)/2 , y2],
       [y1, (y1 + y2)/2 , y2],
       [y1, (y1 + y2)/2 , y2]])
    
    Z = np.array(([lower,lower,lower],
             [(lower + upper)/2, (lower + upper)/2 ,(lower + upper)/2],
             [upper,upper,upper]))
    
    return X,Y,Z

def add_fig(fig,plot_list,h):
    for i,_ in enumerate(plot_list[:] + plot_list[:1]): 
        if i+1 < len(plot_list):
        #h = get_box(fdp_volume['S1NL01']['level'])
            p1,p2 = plot_list[i],plot_list[i+1]
            X,Y,Z = gen_plane(p1,p2,h)
            
            fig.add_traces(data=go.Surface(x=X, y=Y, z=Z,opacity=0.5,colorscale='Blues'))
    fig.update(layout_coloraxis_showscale=False)
    
    return None

def add_top_bot(fig,plot_list,h):
...
def create_3d_plot(volume_name,selected_sector):
    center = find_center_3d(selected_sector)
    plot_list = create_plot_list(volume_name,center)

    h = get_h(volume_name)


    fig = go.Figure()
    add_fig(fig,plot_list,h)
    add_top_bot(fig,plot_list,h)
...
def get_h(volume_name):
    h = []
    if get_box(fdp_volume[volume_name]['level'])[0] =='GND':
        h.append(0)
    else:
        h.append(int(get_box(fdp_volume[volume_name]['level'])[0].replace(',',''))/1000)
    h.append(int(get_box(fdp_volume[volume_name]['level'])[1].replace(',',''))/1000)
    return h

def create_plot_list(volume_name,center):
    plot_list = []
    polygon = get_polygon(volume_name)
    for i in polygon:
        plot_list.append(latlon2xy(list(center.values()),i))

    return plot_list

def find_center_3d(selected_sector):
    volume_list = sector_json[selected_sector]['volume']
    polygon_list = []
    ...
    lat,lon = get_latlon(polygon_list)
    center,_,_ = find_center_sector(lat,lon)
